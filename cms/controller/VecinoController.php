...
class VecinoController
{
    ...
    public function activar($id){
        ...
        if ($vecino->activo == 1){
            ...
            //Desactivo la vecino
            $consulta = $this->db->exec("UPDATE vecinos SET activo=0 WHERE id='$id'");
            ...
            //Activo la vecino
            $consulta = $this->db->exec("UPDATE vecinos SET activo=1 WHERE id='$id'");
    ...
    public function home($id){
        ...
        if ($vecino->home == 1){
            ...
            $consulta = $this->db->exec("UPDATE vecinos SET home=0 WHERE id='$id'");
            ...
            $consulta = $this->db->exec("UPDATE vecinos SET home=1 WHERE id='$id'");
    ...
    public function editar($id){
        ...
        if (isset($_POST["guardar"])){
            ...
            $imagen_subida = ($_FILES['imagen']['name']) ? '/var/www/html'.$_SESSION['img']."img/".$_FILES['imagen']['name'] : "";
            $texto_img = ""; //Para el mensaje
            if ($id == "nuevo"){

                //Creo una nueva vecino
                $consulta = $this->db->exec("INSERT INTO vecinos 
                    (nombre, especie, personalidad, hobby, muletilla, slug, imagen) VALUES 
                    ('$nombre','$especie','$personalidad','$hobby','$muletilla','$slug','$imagen')");
                ...
                //Mensaje y redirección
                $this->view->redireccionConMensaje("admin/vecinos","green","El vecino <strong>$nombre</strong> se guardado correctamente.".$imagen_subida);
